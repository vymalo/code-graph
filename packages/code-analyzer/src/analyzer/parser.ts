...
import {AstNode, FileNode, RelationshipInfo, SingleFileParseResult} from './types.js';
// Import FileNode
            ...
            try {
                switch (file.extension) {
                    ...
                    case '.cjs':
                        ...
                        break;
                    default:
                        const supportedNonSql = config.supportedExtensions.filter((ext: string) => ext !== '.sql');
    ...
    private async _parseTsProjectFiles(targetFiles: Set<string>): Promise<void> {
        ...
        for (const sourceFile of this.tsProject.getSourceFiles()) {
            ...
            const context = { // Create ParserContext
                ...
                resolveImportPath: (source: string, imp: string) => { /* TODO: Implement proper import resolution */
                    return imp;
                },
                now: now,
            };
